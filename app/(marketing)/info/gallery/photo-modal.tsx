...
export function PhotoModal({ isOpen, onClose, photos, initialPhotoId }: PhotoModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <div className={`fixed inset-0 z-50 bg-black/90 ${isFullscreen ? "h-screen w-screen" : ""}`}>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              exit={{ opacity: 0, x: -20 }}
              className={`relative ${isFullscreen ? "w-screen h-screen" : "aspect-video w-full max-w-5xl"}`}
            >
              <Image
